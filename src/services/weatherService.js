...
const API_KEY = "6e4f28574c1db0d04dd1684c177a6fcc";
...
const formatForecastWeather = (secs, offset, data) => {
  const hourly = data.filter((f) => f.dt > secs).map((f) => ({
    temp: f.main.temp,
    time: formatToLocalTime(f.dt, offset, "hh:mm a"), // Add formatted time
  ...
  const daily = data.filter((f) => f.dt_txt.slice(-8) === "00:00:00").map((f) => ({
    ...
    icon: iconUrlFromCode(f.weather[0].icon),
    date: f.dt_txt,
    day: formatToLocalTime(f.dt, offset, "ccc"), // Add day of the week
  }));
